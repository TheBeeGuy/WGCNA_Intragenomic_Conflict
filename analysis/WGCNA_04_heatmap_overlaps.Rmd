...
```{r brain figures}
brain_overlaps<-read.csv(file = "../output/ovary_moduleDEG_PSGE_overlap_results.csv", header=T)%>%select(-X)
...
min_PSGE<-pmap_dfr(PSGE_pvalues,~data.frame(min_PSGE=min(..2,..3)))
min_DEG<-brain_overlaps$DEG_repstat_pvalue
...
pvalues<-data.frame(PSGE_pvalues%>%select(-module_name),
                    brain_overlaps%>%select(DEG_repstat_pvalue,
                    reproductive_pvalue),
                    module_name)%>%filter(
                        module_name!="background")
