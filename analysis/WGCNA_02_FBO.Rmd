...
##can remove the bias filter to include European and Afrianized bias
PSGE_active<-read.csv("../data/PGSE-active.csv")%>%select("ID")
#%>%filter(bias %in% c('maternal','paternal')
...
intersect(PSGE_active$ID,PSGE_active$ID)%>%length()
...
```
```{r DEG PSGE overlaps - hypergeometric tests}
...
##save the resuts out for later. uncomment this to save.
...
#colnames(MEdata_ovary)<-gene_entrez$Entrez
