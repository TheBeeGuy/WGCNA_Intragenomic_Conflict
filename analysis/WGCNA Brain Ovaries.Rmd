...
PSGE_active<-read.csv("PGSE-active.csv")%>%select("ID")
PSGE_sterile<-read.csv("PGSE-sterile.csv")%>%select("ID")
DEG_reproductive<-read.csv("DEG_reproductivestatus_galbraith2016.csv")%>%select("ID")
...
pdf(file = "consensusTraitRelationships-signed.pdf", wi = 6, he = 6);
...
pdf(file = "consensusTraitRelationships-unsigned.pdf", wi = 6, he = 6);
par(mar = c(6, 8.8, 3, 2.2));
labeledHeatmap(Matrix = consensusCor,
xLabels = colnames(consensusCor),
yLabels = rownames(consensusCor),
...
#read in beebase ids
mod_sig_beebase<-read.csv("WGCNA_BvFBO_Sig_Modules_genes_beebase.csv")%>%select(-X)
...
active_parent_overlap<-sapply(mod_sig_beebase,m_intersect,var2=activeDMLgenes)
active_strain_overlap<-sapply(mod_sig_beebase,m_intersect,var2=activeDMLgenes)

sterile_parent_overlap<-sapply(mod_sig_beebase,m_intersect,var2=sterileDMLgenes)
sterile_strain_overlap<-sapply(mod_sig_beebase,m_intersect,var2=sterileDMLgenes)

moduleDML_overlaps<-cbind(module_name,module_size,active_overlap,sterile_overlap)



#Hypergeometric tests for active genes

act_length<-activeDMLgenes%>%unique_length()##length active
ster_length<-sterileDMLgenes%>%unique_length()##length active
background<-14086

#active overlaps
activeDML_pvalue<-c()
for(mod in 1:length(module_name)){
activeDML_pvalue[mod]<-1-phyper(active_overlap[mod], act_length, background-act_length, module_size[mod], lower.tail = TRUE, log.p = FALSE)
}

#sterile overlaps
sterileDML_pvalue<-c()
for(mod in 1:length(module_name)){
sterileDML_pvalue[mod]<-1-phyper(sterile_overlap[mod], ster_length, background-ster_length, module_size[mod], lower.tail = TRUE, log.p = FALSE)
}


##get overlaps for DEGs
active_PSGE_module_overlap<-sapply(mod_sig_beebase,m_intersect,var2=PSGE_active)

sterile_PSGE_module_overlap<-sapply(mod_sig_beebase,m_intersect,var2=PSGE_sterile)

DEG_reproductive_module_overlap<-sapply(mod_sig_beebase,m_intersect,var2=DEG_reproductive)

#length of genes
sterile_PSGE_length<-PSGE_sterile%>%unique_length()##length active psge
active_PSGE_length<-PSGE_active%>%unique_length()#length sterile psge
...
for(mod in 1:length(module_name)){
activePSGE_repstat_pvalue[mod]<-1-phyper(DEG_reproductive_module_overlap[mod], DEG_length, background-DEG_length, module_size[mod], lower.tail = TRUE, log.p = FALSE)
}
...
#creating data sets
moduleDMLgenes_results<-cbind(moduleDML_overlaps,activeDML_pvalue,sterileDML_pvalue)%>%as.data.frame()
all(moduleDEGgenes_results$module_name==moduleDMLgenes_results$module_name)


module_DEG_DML_results<-left_join(moduleDMLgenes_results,moduleDEGgenes_results)




write.csv(moduleDMLgenes_results,"moduleDMLoverlap_results.csv")
write.csv(moduleDEGgenes_results,"moduleDEGoverlap_results.csv")
write.csv(module_DEG_DML_results,"module_DEG_DML_overlap_results.csv") 
...
bin_activeDML<-activeDML_pvalue[-1]<0.05
...
source("C:/Users/Rong/Box\ Sync/Projects/BPO_rnaseq/BPO_rnaseq/find_kegg.R")
...
#read.csv("kegg_pathways_WGCNA_BvFBO.csv")
